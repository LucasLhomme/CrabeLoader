...
class LuaCall {
    public:
        ...
        // Loads `chunk` as a Lua function and calls it with `args` pushed as
        // string parameters, received inside the chunk via `local a, b = ...`.
        // `chunk` is a compile-time constant; no caller data is ever
        // interpolated into it, so a path or a mod name containing a quote, a
        // backslash or a closing parenthesis is data and can never become code.
        bool runChunkWithArgs(void* L,
