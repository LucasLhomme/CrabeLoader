...
namespace crabe::multiplayer::application {
    ...
    bool MultiplayerManager::initialize() {
        ...
        crabe::shared::Logger::getInstance().info("MultiplayerManager: initializing subsystem...");

        // 1. Install WinHttp Redirector
        if (_redirector) {
            auto res = _redirector->install();
            if (!res) {
                crabe::shared::Logger::getInstance().warning("MultiplayerManager: redirector install warning: {}", res.error());
            }
        }
        ...
        _initialized = true;
        crabe::shared::Logger::getInstance().info("MultiplayerManager: initialized successfully.");
        return true;
    }
    ...
    void MultiplayerManager::uninitialize() {
        ...
        _initialized = false;
        crabe::shared::Logger::getInstance().info("MultiplayerManager: uninitialized.");
    }
    ...
    void MultiplayerManager::applyMemoryPatchesNow() {
        if (_patcher) {
            ...
            if (res) {
                crabe::shared::Logger::getInstance().info("MultiplayerManager: synchronously applied memory patches.");
            } else {
                crabe::shared::Logger::getInstance().warning("MultiplayerManager: early memory patch attempt: {}", res.error());
            }
        }
    }
        ...
        for (int i = 0; i < 60 && !_workerStop.load(); ++i) {
            if (_patcher) {
                ...
                if (res && _patcher->arePatchesActive() && _patcher->getPatchedCount() >= 4) {
                    crabe::shared::Logger::getInstance().info("MultiplayerManager: all core memory patches confirmed active (pass {})", i + 1);
                    break;
                }
            }
