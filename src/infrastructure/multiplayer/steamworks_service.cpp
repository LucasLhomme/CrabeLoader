...
namespace crabe::multiplayer {
    ...
    bool SteamworksService::bindSteamFunctions() {
        ...
        if (!_steamApiModule) {
            crabe::shared::Logger::getInstance().warning("SteamworksService: steam_api.dll could not be loaded or located.");
            return false;
        }
    ...
    bool SteamworksService::initialize() {
        ...
        if (_available) {
            ...
            crabe::shared::Logger::getInstance().info("SteamworksService: initialized successfully. Logged in as '{}' (SteamID: {})",
                                       name, getLocalSteamId());
        } else {
            crabe::shared::Logger::getInstance().warning("SteamworksService: initialized but Steam interfaces not yet resolved (is Steam running?).");
        }
    ...
    bool SteamworksService::createLobby(bool friendsOnly, int maxMembers) {
        if (!_pSteamMatchmaking) {
            crabe::shared::Logger::getInstance().warning("SteamworksService::createLobby: ISteamMatchmaking not available.");
            return false;
        }
        ...
        if (callHandle == 0) {
            crabe::shared::Logger::getInstance().warning("SteamworksService::createLobby: CreateLobby API call returned 0.");
            return false;
        }
        ...
        crabe::shared::Logger::getInstance().info("SteamworksService: CreateLobby dispatched (call handle: {})", callHandle);
        return true;
    }
    ...
    void SteamworksService::leaveLobby() {
        ...
        std::lock_guard<std::mutex> lock(_mutex);
        _lobbyStatus = {};
        crabe::shared::Logger::getInstance().info("SteamworksService: left lobby.");
    }
    ...
    bool SteamworksService::joinLobby(uint64_t lobbyId) {
        ...
        crabe::shared::Logger::getInstance().info("SteamworksService: JoinLobby dispatched for lobby {} (call handle: {})", lobbyId, callHandle);
        return true;
    }
    ...
    bool SteamworksService::openInviteOverlay() {
        ...
        if (_lobbyStatus.inLobby && _lobbyStatus.lobbyId != 0 && vtable[27]) {
            ...
            crabe::shared::Logger::getInstance().info("SteamworksService: activated Steam lobby invite dialog for lobby {}", _lobbyStatus.lobbyId);
            return true;
        }
        ...
        if (vtable[22]) {
            ...
            crabe::shared::Logger::getInstance().info("SteamworksService: activated Steam friends overlay.");
            return true;
        }
        return false;
    }
