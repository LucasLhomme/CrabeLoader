...
namespace crabe::multiplayer::infrastructure {
        ...
        HINTERNET hSession, LPCWSTR pswzServerName, INTERNET_PORT nServerPort, DWORD dwReserved)
    {
        ...
        if (shouldRedirectHost(pswzServerName)) {
            ...
            crabe::shared::Logger::getInstance().debug("WinHttpRedirector: redirected Connect to 127.0.0.1:{}", targetPort);
        }
    ...
    std::expected<void, std::string> WinHttpRedirector::install() {
        ...
        s_installed = true;
        crabe::shared::Logger::getInstance().info("WinHttpRedirector: hooked all 7 WinHttp APIs -> 127.0.0.1:{}", s_targetPort);
        return {};
    }
    ...
    void WinHttpRedirector::uninstall() {
        ...
        s_installed = false;
        crabe::shared::Logger::getInstance().info("WinHttpRedirector: unhooked WinHttp APIs");
    }
