...
Deploy by copying this mod folder into the game's `mods/`. The loader reads
`mods/<name>/characters/*.lua`; a `characters/` folder at the game root is not
read any more.
...
`gateway::allocateSku` in `src/application/gateway.cpp` — see
...
unconditionally from `src/api/20_hooks.lua`; callers do not need
...
**The loader supplies that slot itself.** `src/application/gateway.cpp` reimplements the
...
2. Deploy this mod folder into the game's `mods/`.
3. Launch.
...
**Step 3 — deploy** this mod folder into the game's `mods/`, and `skilltrees/`
...
| Tile shows as locked | `installGridUnlock` did not run | it runs from `src/api/20_hooks.lua`, which is embedded in the DLL; check the log for API-load errors |
...
Multiply-and-add only, no XOR, so `src/application/gateway.cpp` and
